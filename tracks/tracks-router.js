...
router.get('/:id', async (req, res) => {
	try {
		...
		res.status(500).json({ message: 'We ran into an error retrieving the track' });
	}
});
...
router.post('/', async (req, res) => {
	...
	if (track.name) {
		try {
			...
			res.status(500).json({ message: 'We ran into an error creating the track' });
		}
	} else {
...
router.put('/:id', async (req, res) => {
	...
	if (changes.name) {
		try {
			...
			res.status(500).json({ message: 'We ran into an error updating the track' });
		}
	} else {
		res.status(400).json({
...
router.delete('/:id', async (req, res) => {
	try {
		...
		res.status(500).json({ message: 'We ran into an error removing the track' });
	}
});
